...
describe("KalmanTempNode", () => {
    ...
    it("should register the node type", () => {
        ...
        module(RED);
        expect(RED.nodes.registerType).toHaveBeenCalledWith("kalman-temp", expect.any(Function));
    });
    ...
    it("should initialize the Kalman filter on first input", () => {
        ...
        const config: Partial<KalmanTempNodeDef> = { id: "1", type: "kalman-temp", name: "test", R: 0.2, Q: 0.001, predictInterval: 60 };
        ...
        const inputCallback = node.on.mock.calls.find(([event]: [string, () => void]) => event === "input")[1];
        inputCallback({ payload: 25 });
        const closeCallback: () => void = node.on.mock.calls.find(([event]: [string, () => void]) => event === "close")[1];
        closeCallback();
